	...
	jQuery.ajax({
		...
		if (json_reponse.length > 0) {
			...
			var item           = json_reponse[i],
				...
				compare_price  = item.has_compare_price
									? '<span class="price-compare">' + item.compare_price + '</span>'
									: '',
				...
				template =
					...
					'</a>' +
					'<a class="search-item-title" href="' + handle + '">' +
						markedString +
					'</a>' +
					compare_price +
			...
			setTimeout(function () {
			result_element
				.children('li.result-item.just-added')
				.removeClass('just-added')
				.slideDown(600);
			}, 300);

			if (result_element.children('li').length) {
			result_wrapper.show();
			currenciesCallbackSpecial(
				result_wrapper.find('span.money')
			);
			}
			...
			result_element.html(
			'<li class="result-item"><p>No result found for your search.</p></li>'
			);
			result_wrapper.show();
		}
		}
...
,ajaxSearch : function (bc_search_config) {
  ...
  var ajax_timeout,
      ...
      search_input_id = (
        bc_search_config.search_input &&
        bc_search_config.search_input.length > 0
      ) ? bc_search_config.search_input : '#bc-product-search',

      // mobile search input
      search_input_id_mobile = (
        bc_search_config.search_input_mobile &&
        bc_search_config.search_input_mobile.length > 0
      ) ? bc_search_config.search_input_mobile : '#bc-product-search-mobile',

      // combine both inputs
      search_inputs = search_input_id + ', ' + search_input_id_mobile,

      // result wrapper
      wrapper_id = (
        bc_search_config.result_wrapper &&
        bc_search_config.result_wrapper.length > 0
      ) ? bc_search_config.result_wrapper : '#result-ajax-search',

      // result list
      result_id = (
        bc_search_config.result_element &&
        bc_search_config.result_element.length > 0
      ) ? bc_search_config.result_element : '.search-results';


  /* =========================
     KEYUP (DESKTOP + MOBILE)
     ========================= */
  jQuery(document).on('keyup', search_inputs, function () {
    ...
    ajax_timeout = setTimeout(function () {
      ...
      if (_keyword.length >= 2) {
        ...
        search_element
          .removeClass('error warning valid')
          .addClass('valid');

        jQuery('.searchbox').addClass('s-loading');

        ajax_search.ajaxProductItems(
          search_element,
          result_wrapper,
          result_element
        );
        ...
        result_element.html(
          '<li><p>You must enter at least 2 characters.</p></li>'
        );
      }
  ...
  /* =========================
     BLUR (HIDE RESULT)
     ========================= */
  jQuery(document).on('blur', '#search.navbar-form', function () {
...
jQuery(document).ready(function($) {
  AT_AjaxSearch.init({
      ...
      ,"search_input_mobile" : "#bc-product-search-mobile"
	  ,"result_wrapper": "#result-ajax-search, #result-ajax-search-mobile"
			...
			jQuery('#' + config.wrapperId),{
				...
				,'transitionOut': 'fade'
              	,'afterShow': function(){
					...
					Currency.convertAll('EGP' , Currency.currentCurrency);
